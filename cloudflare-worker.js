...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .gh-link svg { fill: #8b949e; transition: fill .15s; }
  .gh-link:hover svg { fill: #f0f6fc; }
</style>
</head>
<body>
<div class="card" id="app">
  <div style="display:flex;justify-content:space-between;align-items:flex-start">
    ...
    <a class="gh-link" href="https://github.com/muzimu217/ocusage" target="_blank" rel="noopener" title="GitHub 仓库" aria-label="GitHub 仓库">
      <svg viewBox="0 0 16 16" width="24" height="24" aria-hidden="true"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8z"/></svg>
    </a>
  </div>
...
// 调用链：同源后端(/api/check —— 本地部署时密钥不出本机) > 内置公共转发(静态托管访客兜底) > 浏览器直连
// DEFAULT_PROXY 仅维护者页面托管在无后端的静态环境时使用；本地拉取运行不会用到它。
let lastVia = '';
async function callAPI(key) {
  const candidates = [{ url: PROXY_PATH, via: '本机后端' }];
  if (DEFAULT_PROXY) candidates.push({ url: DEFAULT_PROXY.replace(/\\/+$/, '') + PROXY_PATH, via: '公共转发 ' + DEFAULT_PROXY.replace(/^https?:\\/\\//, '') });

  for (const c of candidates) {
    try {
      // 静态托管（GitHub Pages）会对 POST /api/check 回 404/405，
      ...
      if (r.status !== 404 && r.status !== 405) { lastVia = c.via; return r; }
...
async function checkUsage() {
  ...
  try {
    ...
    // 透明化：显示本次查询实际经过的链路（本机后端 / 公共转发 / 直连）
    document.getElementById('viaInfo').textContent = ' · 本次经 ' + lastVia;
